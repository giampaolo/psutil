...
psutil_kinfo_proc(pid_t pid, struct kinfo_proc *proc) {
    ...
    if (sysctl((int *)mib, 4, proc, &size, NULL, 0) == -1) {
        psutil_SetFromOSErrnoWithSyscall("sysctl(KERN_PROC_PID)");
        return -1;
    }
...
psutil_get_proc_list(struct kinfo_proc **procList, size_t *procCount) {
    ...
    if (err == -1) {
        psutil_SetFromOSErrnoWithSyscall("sysctl (null buffer)");
        return 1;
    }
    ...
    while (1) {
        ...
        if (err == -1) {
            ...
            psutil_SetFromOSErrnoWithSyscall("sysctl()");
            return 1;
        }
        else {
            break;
...
psutil_proc_cmdline(PyObject *self, PyObject *args) {
    ...
    if (sysctl(mib, 4, procargs, &size, NULL, 0) == -1) {
        psutil_SetFromOSErrnoWithSyscall("sysctl(KERN_PROC_ARGS)");
        goto error;
    }
...
psutil_proc_exe(PyObject *self, PyObject *args) {
    ...
    if (error == -1) {
        ...
        if (errno == ENOENT) {
            ...
            return \
                psutil_SetFromOSErrnoWithSyscall("sysctl(KERN_PROC_PATHNAME)");
        }
    }
...
psutil_proc_threads(PyObject *self, PyObject *args) {
    ...
    if (error == -1) {
        psutil_SetFromOSErrnoWithSyscall("sysctl(KERN_PROC_INC_THREAD)");
        goto error;
    }
    if (size == 0) {
        NoSuchProcess("sysctl (size = 0)");
