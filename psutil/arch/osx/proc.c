    ...
    *procCount = 0;
    ...
    while (lim-- > 0) {
        ...
        if (sysctl((int *)mib, 3, NULL, &size, NULL, 0) == -1) {
            psutil_SetFromOSErrnoWithSyscall("sysctl(KERN_PROC_ALL)");
            return 1;
        }
...
psutil_sysctl_argmax() {
    ...
    if (sysctl(mib, 2, &argmax, &size, NULL, 0) == 0)
        return argmax;
    psutil_SetFromOSErrnoWithSyscall("sysctl(KERN_ARGMAX)");
    return 0;
}
...
psutil_sysctl_procargs(pid_t pid, char *procargs, size_t *argmax) {
    ...
    if (sysctl(mib, 3, procargs, argmax, NULL, 0) < 0) {
        ...
        psutil_SetFromOSErrnoWithSyscall("sysctl(KERN_PROCARGS2)");
        return 1;
    }
    return 0;
}
    ...
    for (i = 0; i < num_fds; i++) {
        ...
        if (fdp_pointer->proc_fdtype == PROX_FDTYPE_SOCKET) {
            ...
            if ((family == AF_INET) || (family == AF_INET6)) {
                ...
                if (errno != 0) {
                    psutil_SetFromOSErrnoWithSyscall("inet_ntop()");
                    goto error;
                }
