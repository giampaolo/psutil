...
psutil_proc_kill(PyObject *self, PyObject *args) {
    ...
    if (! TerminateProcess(hProcess, SIGTERM)) {
        ...
        if (GetLastError() != ERROR_ACCESS_DENIED) {
            psutil_SetFromOSErrnoWithSyscall("TerminateProcess");
            return NULL;
        }
    }
...
psutil_proc_wait(PyObject *self, PyObject *args) {
    ...
    if (hProcess == NULL) {
        if (GetLastError() == ERROR_INVALID_PARAMETER) {
            ...
            psutil_SetFromOSErrnoWithSyscall("OpenProcess");
            return NULL;
        }
    }
    ...
    if (retVal == WAIT_FAILED) {
        psutil_SetFromOSErrnoWithSyscall("WaitForSingleObject");
        CloseHandle(hProcess);
        return NULL;
    }
    ...
    if (GetExitCodeProcess(hProcess, &ExitCode) == 0) {
        psutil_SetFromOSErrnoWithSyscall("GetExitCodeProcess");
        CloseHandle(hProcess);
        return NULL;
    }
...
psutil_proc_threads(PyObject *self, PyObject *args) {
    ...
    if (hThreadSnap == INVALID_HANDLE_VALUE) {
        psutil_SetFromOSErrnoWithSyscall("CreateToolhelp32Snapshot");
        goto error;
    }
    ...
    if (! Thread32First(hThreadSnap, &te32)) {
        psutil_SetFromOSErrnoWithSyscall("Thread32First");
        goto error;
    }
    ...
    do {
        if (te32.th32OwnerProcessID == pid) {
            ...
            rc = GetThreadTimes(hThread, &ftDummy, &ftDummy, &ftKernel,
                                &ftUser);
            if (rc == 0) {
                psutil_SetFromOSErrnoWithSyscall("GetThreadTimes");
                goto error;
            }
...
_psutil_user_token_from_pid(DWORD pid) {
    ...
    if (!OpenProcessToken(hProcess, TOKEN_QUERY, &hToken)) {
        psutil_SetFromOSErrnoWithSyscall("OpenProcessToken");
        goto error;
    }
    ...
    while (1) {
                                 ...
                                 &bufferSize))
        {
            if (GetLastError() == ERROR_INSUFFICIENT_BUFFER) {
                ...
                psutil_SetFromOSErrnoWithSyscall("GetTokenInformation");
                goto error;
            }
        }
        break;
    ...
    while (1) {
                               ...
                               domainName, &domainNameSize, &nameUse))
        {
            if (GetLastError() == ERROR_INSUFFICIENT_BUFFER) {
                ...
                psutil_SetFromOSErrnoWithSyscall("LookupAccountSidW");
                goto error;
            }
        }
        break;
