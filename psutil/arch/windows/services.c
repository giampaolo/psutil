...
psutil_get_service_handler(char *service_name, DWORD scm_access, DWORD access)
{
    ...
    if (sc == NULL) {
        psutil_SetFromOSErrnoWithSyscall("OpenSCManager");
        return NULL;
    }
    hService = OpenService(sc, service_name, access);
    if (hService == NULL) {
        psutil_SetFromOSErrnoWithSyscall("OpenService");
...
psutil_winservice_query_config(PyObject *self, PyObject *args) {
    ...
    if (GetLastError() != ERROR_INSUFFICIENT_BUFFER) {
        psutil_SetFromOSErrnoWithSyscall("QueryServiceConfigW");
        goto error;
    }
...
psutil_winservice_query_status(PyObject *self, PyObject *args) {
    ...
    if (GetLastError() != ERROR_INSUFFICIENT_BUFFER) {
        psutil_SetFromOSErrnoWithSyscall("QueryServiceStatusEx");
        goto error;
    }
...
psutil_winservice_query_descr(PyObject *self, PyObject *args) {
    ...
    if (GetLastError() != ERROR_INSUFFICIENT_BUFFER) {
        psutil_SetFromOSErrnoWithSyscall("QueryServiceConfig2W");
        goto error;
    }
...
psutil_winservice_start(PyObject *self, PyObject *args) {
    ...
    if (ok == 0) {
        psutil_SetFromOSErrnoWithSyscall("StartService");
        goto error;
    }
...
psutil_winservice_stop(PyObject *self, PyObject *args) {
    ...
    if (ok == 0) {
        psutil_SetFromOSErrnoWithSyscall("ControlService");
        goto error;
    }
