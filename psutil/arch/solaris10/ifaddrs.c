...
int getifaddrs (struct ifaddrs **ifap)
{
  ...
  *ifap = NULL;
  ...
  lifn.lifn_flags = 0;
  if (ioctl(sd, SIOCGLIFNUM, &lifn) < 0) goto error;
  ...
  if (ioctl(sd, SIOCGLIFCONF, &ifc) < 0) goto error;
  ...
  while (ccp < ecp) {
    ...
    if (ioctl(sd, SIOCGLIFADDR, ifr, IFREQSZ) < 0) goto error;
    cifa->ifa_addr = sa_dup((struct sockaddr*)&ifr->lifr_addr);

    if (ioctl(sd, SIOCGLIFNETMASK, ifr, IFREQSZ) < 0) goto error;
  ...
  return 0;
error:
  if (ifc.lifc_buf != NULL) free(ifc.lifc_buf);
  if (sd != -1) close(sd);
  return (-1);
}
